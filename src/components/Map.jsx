...
class Map extends Component {
  ...
  createMarker = (map, points, index) => {
    const pinImage = new google.maps.MarkerImage('http://chart.apis.google.com/chart?chst=d_map_pin_letter&chld=%E2%80%A2|' + this.getPinColor(this.state.courts[index]),
  ...
  setMarkerContent = (index) => {
    const type = this.state.courts[index].Type.toLowerCase();
  ...
  componentDidUpdate() {
    ...
    if (this.state.courts.length > 0) {
      ...
      for (let j = 0; j < points.length; j++) {
        ...
        let marker = that.createMarker(map, points, j);
        let infowindow = that.setMarkerContent(j);
